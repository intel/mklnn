...
local mklOP2thOP = {}
mklOP2thOP['nn.SpatialConvolutionMKLDNN'] 	= nn.SpatialConvolution
mklOP2thOP['nn.SpatialConvolution']	 	= nn.SpatialConvolution
mklOP2thOP['nn.SpatialMaxPoolingMKLDNN'] 	= nn.SpatialMaxPooling
mklOP2thOP['nn.SpatialMaxPooling']	 	= nn.SpatialMaxPooling
mklOP2thOP['nn.SpatialAveragePoolingMKLDNN']	= nn.SpatialAveragePooling
mklOP2thOP['nn.SpatialAveragePooling']		= nn.SpatialAveragePooling
mklOP2thOP['nn.LRNMKLDNN'] 			= nn.SpatialCrossMapLRN
mklOP2thOP['nn.SpatialCrossMapLRN'] 		= nn.SpatialCrossMapLRN
mklOP2thOP['nn.ReLUMKLDNN'] 			= nn.ReLU
mklOP2thOP['nn.ReLU']	 			= nn.ReLU
mklOP2thOP['nn.ConcatMKLDNN'] 			= nn.Concat
mklOP2thOP['nn.Concat'] 			= nn.Concat
mklOP2thOP['nn.View'] 				= nn.View


local thOP2mklOP = {}
thOP2mklOP['nn.SpatialConvolution'] 		= nn.SpatialConvolutionMKLDNN
thOP2mklOP['nn.SpatialConvolutionMKLDNN'] 	= nn.SpatialConvolutionMKLDNN
thOP2mklOP['nn.SpatialMaxPooling']	 	= nn.SpatialMaxPoolingMKLDNN
thOP2mklOP['nn.SpatialMaxPoolingMKLDNN'] 	= nn.SpatialMaxPoolingMKLDNN
thOP2mklOP['nn.SpatialAveragePooling']		= nn.SpatialAveragePoolingMKLDNN
thOP2mklOP['nn.SpatialAveragePoolingMKLDNN']	= nn.SpatialAveragePoolingMKLDNN
thOP2mklOP['nn.SpatialCrossMapLRN'] 		= nn.LRNMKLDNN
thOP2mklOP['nn.LRNMKLDNN']		 	= nn.LRNMKLDNN
thOP2mklOP['nn.ReLU'] 				= nn.ReLUMKLDNN
thOP2mklOP['nn.ReLUMKLDNN'] 			= nn.ReLUMKLDNN
thOP2mklOP['nn.Concat'] 			= nn.ConcatMKLDNN
thOP2mklOP['nn.ConcatMKLDNN'] 			= nn.ConcatMKLDNN
...
function modelTransform(src_model, th2mkl)
    local cvtOp = (th2mkl == 0) and thOP2mklOP or mklOP2thOP
    return convertModel(src_model, cvtOp)
end

function modelAdvancedTransform(src_model, th2mkl)
    local cvtOp = (th2mkl == 0) and thOP2mklOP or mklOP2thOP
		...
		for i = 1, #src_module do
			...
			elseif(string.find(layer_type, 'Concat') or string.find(layer_type, 'Sequential')) then 
			    ...
			    if(prevOPFlag == 1) then
			        ...
			        local convert_layer = nn.Convert()
